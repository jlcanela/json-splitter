...
object Splitter {
  ...
  val live: ZLayer[Console with S3, Nothing, Splitter] = ZLayer.succeed {
    new Service {
      ...
      def writeS3Data(zs: ZStream[Blocking with S3 with Console, Throwable, Byte], bucket: String, path: String) = for {
      ...
      def writeFileData(zs: ZStream[Any, Throwable, Byte], path: Path) = for {
        _ <- ZIO.fromOption(Option(path.getParent)).map(path => Files.createDirectories(path)) orElse ZIO.succeed(())
      ...
      def createQueue(destination: JPath, name: String) = for {
        ...
        _ <- destination match {
          ...
          case S3File(bucket: String, path: String) => writeS3Data(zs, bucket, s"${path}/${name}").fork
        }
      } yield queue
      ...
      def createS3Stream(path: S3File): ZStream[S3, Throwable, String] = {
        s3.getObject(path.bucket, path.path)
          .transduce(ZTransducer.utfDecode)
          .transduce(ZTransducer.splitLines)
      }

      def createLocalStream(path: Path): ZStream[Blocking, Throwable, String] =
        ZStream
          .fromFile(path)
      ...
      def createStream(path: JPath): ZStream[S3 with Blocking, Throwable, String] =
        path match {
          case path: S3File    => createS3Stream(path)
          case LocalFile(path) => createLocalStream(path)
        }
      ...
      def addQueue(destination: JPath)(
          ...
          name: String
      ): ZIO[Blocking with S3 with Console, Throwable, (Map[String, Queue[String]], Queue[String])] = for {
